...
for cmember in cmembers.stripped_strings:
    if (hasattr(cmember.find('img'),'src')):
        cpic = 'https://govtrack.us'+(cmember.find('img').attrs['src'])
    else:
        cpic = 'null'
    cmember = str(cmember.text)
    cname = cmember.splitlines()[4]
    cbio = cmember.splitlines()[5]
    memberinfo.append((cname, cbio, cpic))

#Send info to csv file defined earlier  
df = pd.DataFrame(memberinfo, columns=['Congressmember Name','Congressmember Role', 'Congressmember Picture'])  
